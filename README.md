...
1. Add pre-built script for changing the Debug and Release versions of the framework:
   ...
   * Copy-paste following script:
        ```shell
        cd "${SRCROOT}/../../plugins/com.talsec.cordova.freerasp/src/ios"
...
3. Add dependency on the symlink
   ...
   * Add dependency (drag & drop) on the symlink on the following location:
     *MyApp/plugins/com.talsec.cordova.freerasp/src/ios/TalsecRuntime.xcframework*
...
You might want to react differently to each callback, e.g. kill the application or notify user about the incident. You can do so in the **threatListener**. In the example, it is used to vizualize the results of the Talsec checks:
<h1>
<img src="https://raw.githubusercontent.com/talsec/Free-RASP-Community/master/visuals/cordovaExample.png" width=50%>
</h1>

You can achieve it by modifying following files:
* index.js:
```js
